  ...
  const handleQuerySinter = async (type: 'cib' | 'inscricao', val: string) => {
    ...
    try {
      ...
      if (contentType && contentType.includes('application/json')) {
        ...
        result = { success: false, error: 'Resposta inválida do servidor backend', details: text };
      }
      ...
      if (response.ok && result && result.success && result.data) {
        ...
        setQueryError({
          error: result?.error || `A API do SINTER/SERPRO retornou erro HTTP ${response.status}`,
      ...
      setQueryError({
        error: 'Erro de conexão com o servidor municipal.',
        details: err.message
      });
    } finally {
